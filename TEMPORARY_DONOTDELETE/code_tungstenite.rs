// CE CODE PEUT REMPLACER EWEBSOCKET dans client/main.rs
use notan::prelude::*;
use notan::draw::*;
use shared::*;
...
type NetReceiver = Receiver<String>;
type NetSender = UnboundedSender<String>;
...
struct State {
    ...
    // Nouveaux canaux (remplace ws_sender/receiver)
    net_tx: NetSender,
    net_rx: NetReceiver,
...
// --- Thread Réseau (Architecture Tokio) ---

fn spawn_network_thread(url_str: &str) -> (NetSender, NetReceiver) {
    let (game_tx, mut network_rx) = tokio::sync::mpsc::unbounded_channel::<String>();
    let (network_tx, game_rx) = mpsc::channel::<String>();
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            println!("Tentative de connexion à {}...", url_string);
            // On passe la string directement (fix compatibilité url)
            match connect_async(&url_string).await {
                Ok((ws_stream, _)) => {
                    ...
                    loop {
                        tokio::select! {
                            // 1. Recevoir du serveur
                            msg = read.next() => {
                                match msg {
                                    Some(Ok(Message::Text(utf8_bytes))) => {
                                        // Conversion explicite pour compatibilité tungstenite récente
                                        let text = utf8_bytes.to_string();
                                        if let Err(_) = network_tx.send(text) {
                                            break; 
                                        }
                                    }
                            ...
                            msg_to_send = network_rx.recv() => {
                                match msg_to_send {
                                    Some(text) => {
                                        // Conversion explicite vers Message::Text
            ...
            println!("Arrêt du thread réseau.");
        });
    });
...
fn setup(app: &mut App, gfx: &mut Graphics) -> State {
    // J'ai mis 127.0.0.1 (Localhost) pour que ça marche chez toi.
    // Si tu veux tester avec le vrai serveur distant, remets "ws://59.66.221.76:8080/ws"
    let (net_tx, net_rx) = spawn_network_thread("ws://59.66.221.76:8080/ws");
...
// --- Draw (UI Complète restaurée) ---

fn draw(app: &mut App, gfx: &mut Graphics, state: &mut State) {
    let mut draw = gfx.create_draw();
    draw.clear(Color::from_rgb(0.05, 0.05, 0.05)); 

    // 1. Gestion Réseau (via Channels)
    loop {
        match state.net_rx.try_recv() {
            Ok(text) => {
            ...
            Err(TryRecvError::Disconnected) => {
                state.last_server_msg = "Connexion perdue".to_string();
                break;
            }
        }
    }
    ...
    if can_play {
        ...
        if app.keyboard.was_pressed(KeyCode::R) && (state.board.state == GameState::GameOver || state.board.state == GameState::Paused) {
            ...
            if let Ok(json) = serde_json::to_string(&msg) { let _ = state.net_tx.send(json); }
        }
        ...
        if state.board.state == GameState::Playing {
            ...
            let piece_locked_now = if app.keyboard.was_pressed(KeyCode::Space) || app.keyboard.was_pressed(KeyCode::Return) {
                if let Some(piece) = &state.board.active_piece {
                    ...
                    if let Ok(json) = serde_json::to_string(&action_msg) { let _ = state.net_tx.send(json); }
                }
        ...
        match state.board.state {
            GameState::Playing => {
                ...
                if locked {
                    if let Some(msg) = pending_lock_msg { if let Ok(json) = serde_json::to_string(&msg) { let _ = state.net_tx.send(json); } }
                    state.last_fall_time = time_now;
                } else {
    ...
    // 3. Dessin UI (Tout ton ancien code graphique)
...
// --- Helpers Graphiques (Ta fonction draw_board complète) ---
