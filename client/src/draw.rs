...
pub fn draw_game(app: &mut App, gfx: &mut Graphics, session: &GameSession, font: &Font, is_host: bool) {
    ...
    draw.text(font, "YOU").position(start_x, offset_y - 50.0).size(20.0).color(Color::WHITE);
    draw_nuisance_bar(&mut draw, font, session.board.pending_garbage, start_x, offset_y - 28.0, board_w);

    let opponent_x = start_x + board_w + gap;
    draw_board(&mut draw, &session.other_board, opponent_x, offset_y, board_w, board_h, session.opponent_piece_offset);
    draw.text(font, "OPPONENT").position(opponent_x, offset_y - 50.0).size(20.0).color(Color::GRAY);
    draw_nuisance_bar(&mut draw, font, session.other_board.pending_garbage, opponent_x, offset_y - 28.0, board_w);
    ...
    draw_chain_anim(&mut draw, font, &session.chain_display, start_x, offset_y, board_w, board_h);
    ...
    if session.all_clear_timer > 0.0 {
        ...
        draw.text(font, "ALL CLEAR!")
            ...
            .size(48.0).h_align_center().v_align_middle()
...
// Nuisance bar: one block per 6 nuisance, max 6 blocks (one full row) + text for overflow.
fn draw_nuisance_bar(draw: &mut Draw, font: &Font, nuisance: u32, board_x: f32, bar_y: f32, board_w: f32) {
    if nuisance == 0 { return; }

    let block_w = board_w / config::GRID_WIDTH as f32;  // = CELL_SIZE
    let block_h = 20.0;
    ...
    // Partial block for leftover nuisance < 6
    if leftover > 0 && rocks < config::GRID_WIDTH as u32 {
        ...
        draw.rect((x, bar_y), (partial_w, block_h)).color(Color::from_rgba(color.r, color.g, color.b, 0.5));
    }

    // Show exact count if more than one full row
...
fn draw_chain_anim(
    ...
    board_x: f32, board_y: f32, board_w: f32, board_h: f32,
) {
    let Some((count, t)) = chain_display else { return };
    // Fully visible, fade out in the last 0.5s
    let alpha = (*t / 0.5).min(1.0_f32);
    // Scale punch: 1.4× at start, settles to 1.0 over 0.3s
